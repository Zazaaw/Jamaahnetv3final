...
export default function CalendarScreen({ session, onNavigate, onBack }: { session: any; onNavigate?: (screen: string, data?: any) => void; onBack?: () => void }) {
  const [events, setEvents] = useState<Event[]>([]);
  const [selectedCategory, setSelectedCategory] = useState<string>('Semua');
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [currentUserRole, setCurrentUserRole] = useState<string>('');
  const [loading, setLoading] = useState(false);
  const [viewMode, setViewMode] = useState<'jadwal' | 'kabar'>('jadwal');
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/20 to-purple-50/20 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
      ...
      >
        ...
        <div className="relative z-10 p-6 pb-8">
          ...
          {/* Category Pills - LAMPU HIJAU CUMA BUAT JADWAL */}
          {viewMode === 'jadwal' && (
      ...
      {/* Events List & Kabar Sosial */}
      <div className="relative z-10 p-6 space-y-4">
        
        {/* Toggle Tab Jadwal & Kabar */}
        <div className="flex bg-gray-100/80 dark:bg-gray-800/50 p-1.5 rounded-2xl mb-4 backdrop-blur-sm border border-gray-200 dark:border-gray-700/50">
          <motion.button 
            whileTap={{ scale: 0.97 }}
