...
export default function AuthScreen({ 
  ...
  useEffect(() => {
    const lookupInviterName = async () => {
      ...
      // Query the profiles table for the member_id
      try {
    ...
    // Debounce the lookup to avoid too many queries
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // Validate that we have a valid inviter name
    if (!inviterName) {
    ...
    try {
      // Proceed with signup - store the inviter's NAME, not the code
      const { data, error } = await supabase.auth.signUp({ 
        ...
        options: { 
          data: { 
            name, 
            invited_by: inviterName, // Store the inviter's NAME
      ...
      <div className="min-h-screen flex items-center justify-center p-6 pt-24">
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            >
              {isSignup ? 'Daftar & Bergabung' : 'Masuk'}
            </motion.h1>
